...
@section('title') {{env('APP_CURRENCY')}} Wallet Credit & Debit @endsection

@section('content')
    @include('admin.breadcrumbs', [
         'crumbs' => [
             env('APP_CURRENCY').' Wallet Credit & Debit'
         ]
    ])
    <form method="post" action="{{ route('admin.coin-wallet-transactions.store') }}">
        @csrf
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <div class="table-responsive mt-3">
                        <table class="table table-hover table-bordered table-striped text-nowrap">
                            ...
                            <tr>
                                ...
                                <th>Opening Balance ({{ env('APP_CURRENCY') }})</th>
                                <th>Type</th>
                                <th>Amount ({{ env('APP_CURRENCY') }})</th>
                                <th>Closing Balance ({{ env('APP_CURRENCY') }})</th>
                            </tr>
                            </thead>
                            <tbody>
            ...
            if (code.length >= 6) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data) {
                            $('.transferName').html(
                                ...
                                '<span class="help-block text-primary font-weight-bold">{{ env('APP_CURRENCY') }} Wallet Balance : {{ env('APP_CURRENCY') }} ' + data.coin_wallet_balance + '</span>'
                            );
                        }
                    },
                    error: function (jqXHR) {
                        ...
                        if (jqXHR.status === 404) {
                            $('.transferName').html(
                                '<span class="help-block text-danger font-weight-bold">User not found</span>'
                            );
                        } else {
                            $('.transferName').html(
                ...
                if (code.length > 0) {
                    $('.transferName').html(
                        '<span class="help-block transferName text-danger font-weight-bold">User not found</span>'
                    );
                }
            }
        }
