                    ...
                    <form method="post" action="{{ route('admin.settings.change-logo') }}" class="filePondForm">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="file" name="logo" class="filePondInput"
                                   data-url="{{ settings()->getFileUrl('logo') }}"
                        ...
                        <div class="form-group mb-3">
                            <label>White Logo <span class="text-danger">*</span> <span class="text-danger"> (Width: 260px x Height: 80px)</span></label>
                            <input type="file" name="logo_white" class="filePondInput"
                                   data-url="{{ settings()->getFileUrl('logo_white') }}"
                                   required accept="image/*">
                            @foreach($errors->get('logo_white') as $error)
                                <div class="text-danger">{{ $error }}</div>
                            @endforeach
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" name="favicon" class="filePondInput"
                                   data-url="{{ settings()->getFileUrl('favicon') }}"
                                   required accept="image/*">
